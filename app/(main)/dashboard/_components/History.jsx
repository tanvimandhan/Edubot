...
const History = () => {
  ...
  return (
    <div>
        <div>
            <h2 className='font-bold text-lg'>Your previous lectures</h2>

            {discussionRoomList?.length==0&&<h2 className='text-gray-400'>no prev lectures watched</h2>}
            <div className='mt-5'>
              {discussionRoomList.map((item,index)=>(item.coachingOption=='Topic based lecture'||item.coachingOption=='Learn language' || item.coachingOption=='Topic based lecture'||item.coachingOption=='Topic based lecture'||item.coachingOption=='Meditation')(
                <div key={index} className='border-b-[2px] pb-3 mb-4 group flex justify-between items-center cursor-pointer'>
                  ...
                  <Link href={'/view-summary/'+item._id}>
                     <Button variant='outline' className='invisible group-hove:visible'>View Notes</Button>
                  </Link>
